...
function Clock() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      setHoursAngle((newTime.getHours() % 12) * 30 + newTime.getMinutes() * 0.5);
  ...
  return (
    <div className={styles.clockContainer}>
      <div className={styles.clockFace}>
        <div className={styles.hourPointer} style={{ transform: `rotate(${hoursAngle}deg)` }} />
        <div className={styles.minutePointer} style={{ transform: `rotate(${minutesAngle}deg)` }} />
        <div className={styles.secondPointer} style={{ transform: `rotate(${secondsAngle}deg)` }} />
      </div>
    </div>
  );
}
